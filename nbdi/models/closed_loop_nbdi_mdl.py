...
from nbdi.utils.general_utils import batch_apply, ParamDict
from nbdi.utils.pytorch_utils import get_constant_parameter, ResizeSpatial, RemoveSpatial
from nbdi.models.skill_prior_mdl import SkillPriorMdl, ImageSkillPriorMdl
from nbdi.modules.subnetworks import Predictor, BaseProcessingLSTM, Encoder
...
class ClNBDIRLMdl(SkillPriorMdl):
    """NBDI model with closed-loop low-level skill decoder."""
...
class ImageClNBDIRLMdl(ClNBDIRLMdl, ImageSkillPriorMdl):
    """NBDI model with closed-loop decoder that operates on image observations."""
    ...
    def _build_inference_net(self):
        self.img_encoder = nn.Sequential(ResizeSpatial(self._hp.prior_input_res),  # encodes image inputs
                                         ...
                                         RemoveSpatial(),)
        return ClNBDIRLMdl._build_inference_net(self)
