...
from nbdi.components.base_model import BaseModel
from nbdi.components.logger import Logger
from nbdi.components.checkpointer import load_by_key, freeze_modules
from nbdi.modules.losses import KLDivLoss, NLL
from nbdi.modules.subnetworks import BaseProcessingLSTM, Predictor, Encoder
from nbdi.modules.recurrent_modules import RecurrentPredictor
from nbdi.utils.general_utils import AttrDict, ParamDict, split_along_axis, get_clipped_optimizer
from nbdi.utils.pytorch_utils import map2np, ten2ar, RemoveSpatial, ResizeSpatial, map2torch, find_tensor, \
                                        TensorModule, RAdam
from nbdi.utils.vis_utils import fig2img
from nbdi.modules.variational_inference import ProbabilisticModel, Gaussian, MultivariateGaussian, get_fixed_prior, \
                                                mc_kl_divergence
from nbdi.modules.layers import LayerBuilderParams
from nbdi.modules.mdn import MDN, GMM
from nbdi.modules.flow_models import ConditionedFlowModel


class SkillPriorMdl(BaseModel, ProbabilisticModel):
    """Skill embedding + prior model for NBDI algorithm."""
    ...
    def forward(self, inputs, use_learned_prior=False):
        """Forward pass of the NBDI model.
    ...
    def loss(self, model_output, inputs):
        """Loss computation of the NBDI model.
        :arg model_output: output of NBDI model forward pass
        :arg inputs: dict with 'states', 'actions', 'images' keys from data loader
        """
        losses = AttrDict()
        inputs.action_mask = inputs.action_mask[:,:inputs.action_mask.shape[1],:]
    ...
    def _log_outputs(self, model_output, inputs, losses, step, log_images, phase, logger, **logging_kwargs):
        """Optionally visualizes outputs of NBDI model.
        :arg model_output: output of NBDI model forward pass
        :arg inputs: dict with 'states', 'actions', 'images' keys from data loader
        :arg losses: output of NBDI model loss() function
