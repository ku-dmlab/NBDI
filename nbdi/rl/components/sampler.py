...
from nbdi.utils.general_utils import listdict2dictlist, AttrDict, ParamDict, obj2np
from nbdi.modules.variational_inference import MultivariateGaussian
from nbdi.rl.utils.reward_fcns import sparse_threshold
...
class Sampler:
    ...
    def sample_action(self, obs, reset_steps_since_hl=False):
        
        if reset_steps_since_hl :
            return self._agent.act(obs, reset_steps_since_hl = True)  # give fake obs
            ...
            with self._agent.val_mode() if not is_train else contextlib.suppress():
                with self._agent.rollout_mode():
                    while not done and self._episode_step < self._max_episode_len:
                        ...
                        obs, reward, done, info = self._env.step(agent_output.action[:9])
                        obs = self._postprocess_obs(obs)
...
class HierarchicalSampler(Sampler):
    ...
    def sample_batch(self, batch_size, is_train=True, global_step=None, store_ll=True, epoch=-1):
            ...
            with self._agent.val_mode() if not is_train else contextlib.suppress():
                with self._agent.rollout_mode():
                    while hl_step < batch_size or len(ll_experience_batch) <= 1:
                        ...
                        acts_for_icm.append(agent_output.action)
                        obs_for_icm.append(obs)

                        if agent_output.action[9] > 0 :
                            agent_output.is_hl_step = True
                            self.sample_action(obs=1, reset_steps_since_hl=True)  # give fake obs. reset steps_since_hl
                        ...
                        if agent_output.is_hl_step or (done or self._episode_step >= self._max_episode_len-1):
                            end_token = [i for i, x in enumerate(acts_for_icm) if x[9] > 0]
                            if len(end_token) == 0:
                                ICM_skill_len = 30
                            else :
                                ICM_skill_len = end_token[0] + 1 
                                
                            if len(obs_for_icm) < ICM_skill_len :
                                obs_next = np.array(obs_for_icm[-1])
                            else : 
                                obs_next = np.array(obs_for_icm)[ICM_skill_len-1] 
                            ...
                            if self.last_hl_obs is not None and self.last_hl_action is not None:
                                hl_experience_batch.append(AttrDict(
                                    ...
                                    reward=np.sum(self.reward_since_last_hl[:ICM_skill_len]),   
                                    done=done,
                                    action=self.last_hl_action,
                                    observation_next=obs_next,
                                    skill_length=np.array(ICM_skill_len)
                                ))
                            ...
                            self.reward_since_last_hl = []

                        # update stored observation
                        self._obs = obs
                        env_steps += 1; self._episode_step += 1; self._episode_reward += reward
                        self.reward_since_last_hl.append(reward)
